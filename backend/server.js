...
app.patch('/api/lists/:id', validateSchema(Schemas.UpdateListSchema), authorize(List), async (req, res, next) => {
    try {
        ...
        if (updates.boardId) {
            const board = await Board.findOne({ _id: updates.boardId, user_id: req.user.id, deleted_at: null });
            if (!board) return res.status(404).json({ error: 'Target board not found' });
            updates.board_id = updates.boardId;
            delete updates.boardId;
        }
        const list = await List.findOneAndUpdate({ _id: req.params.id }, updates, { new: true });
        res.json(list);
    } catch (err) { next(err); }
});
...
app.patch('/api/tasks/:id', validateSchema(Schemas.UpdateTaskSchema), authorize(Task), async (req, res, next) => {
    try {
        const updates = { ...req.body };
        // Normalize list move (accept camelCase or snake_case) + denormalize board_id
        const targetListId = updates.listId || updates.list_id;
        if (targetListId) {
            const targetList = await List.findOne({ _id: targetListId, user_id: req.user.id, deleted_at: null });
            if (!targetList) return res.status(404).json({ error: 'Target list not found' });
            updates.list_id = targetListId;
            updates.board_id = targetList.board_id;
        }
        delete updates.listId;
